...
points(shock_indices, data_tsibble$detrend[shock_indices], col = "red", pch = 19)
legend("topright", 
       legend = "Shocks Identified", 
       col = "red", 
       pch = 19,
       cex = 0.8)
...
#Train-test split 
train <- head(data_tsibble$exrate_rm, round(length(data_tsibble$exrate_rm) * 0.7))
...
#random walk 
ex_rate.train_rw <- Arima(train, order=c(0,1,0) , lambda=lambda) #guerrero technique 
...
ex_rate.train <- Arima(train, order=c(1,1,2), #damped-trend linear exponential smoothing:
                       lambda=lambda) #what is seasonal? 
...
ex_rate.train %>%
  ...
  ylab("Box-Cox Transformed Exchange Rate")+
  autolayer(test_ts, series = "Real Exchange Rate") +  # Add test data as a new series
  ggtitle("Forecasts for Exchange Rate from ARIMA(1,1,2) with drift") +
  xlab("Time") +
  ylab("Exchange Rate") +
  guides(
    color = guide_legend(title = "Series"), 
